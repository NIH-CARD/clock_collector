...
def camera_decision(server_ok: bool, client_status: Optional[str]):
    ...
    Returns ``(show_camera, message_or_None)``. When the client status is pending
    or unknown we optimistically show the camera so a working one is never hidden.
    """
    if not server_ok:
        return (False, CAMERA_MSG_HTTPS)
    if client_status in (None, "ok"):
        return (True, None)
    if client_status == "insecure":
        return (False, CAMERA_MSG_HTTPS)
    if client_status == "nocam":
        return (False, CAMERA_MSG_NO_DEVICE)
    return (False, CAMERA_MSG_UNAVAILABLE)  # unsupported / error / unexpected
